# Crop Planning Plugin - E2E Manual Test

Complete end-to-end test for adaptive crop planning.
...
**Verify:**
```bash
grep -c "crop_planning" .claude/settings.local.json
```
Should output: `9`
...
- Invokes `/crop-planning:requirement-analysis` skill
...
**Setup:**
```bash
echo "invalid json" > .claude/crop_planning/sessions/test-session.json
```
...
**Cleanup:**
```bash
rm .claude/crop_planning/sessions/test-session.json
```
...
```bash
cat .claude/settings.local.json | jq '.permissions.allow[] | select(. | tostring | contains("crop_planning"))'
```
...
8. Skill (crop-planning:requirement-analysis)
9. Read (session data)
...
**Test init:**
```bash
mise run test:e2e:cli:crop-planning:init
```
...
**Test next-question:**
```bash
SESSION_ID=<SESSION_ID> mise run test:e2e:cli:crop-planning:next-question
```
...
rm -rf .claude/crop_planning/sessions/
```

**Optional: Remove permissions (if needed)**
```bash
# Edit .claude/settings.local.json manually to remove crop_planning entries
```
...
Then run `/crop-planning:good-question` again
...
**Create directory:**
```bash
mkdir -p .claude/crop_planning/sessions
```
