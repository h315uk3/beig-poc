#!/usr/bin/env python3
"""
CLI for crop planning session management.
...
Session state is persisted to .claude/crop_planning/sessions/<session_id>.json
"""
...
from crop_planning.lib.dimension_belief import HypothesisSet
from crop_planning.lib.session_orchestrator import SessionOrchestrator
...
def get_session_dir() -> Path:
    ...
    while current not in (current.parent, home):
        claude_dir = current / ".claude" / "crop_planning" / "sessions"
        if claude_dir.parent.parent.exists():
            claude_dir.mkdir(parents=True, exist_ok=True)
            return claude_dir
        current = current.parent

    # Fallback: Use .crop_planning in current directory (no workspace found)
    # This avoids polluting ~/.claude/ when run outside a workspace
    fallback = Path.cwd() / ".crop_planning" / "sessions"
...
def main() -> None:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Crop planning session management CLI")
