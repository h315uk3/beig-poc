# Contributing to crop-planning

This guide covers crop-planning plugin specific development. For general contribution guidelines, see the [root CONTRIBUTING.md](../../CONTRIBUTING.md).

## Plugin Overview

crop-planning is a crop planning assistant plugin that:
...
**Key Components:**
- **Commands**: User-facing slash commands (`/crop-planning:good-question`)
- **Skills**: Computational algorithms (`/crop-planning:requirement-analysis`)
...
**`/crop-planning:good-question` - Crop Planning**
- Test: Start a new planning session
- Verify:
  ...
  5. Check `<workspace>/.claude/crop_planning/question_feedback.json` contains session data

### Skill Test Procedures

**`/crop-planning:requirement-analysis` - Analyze Crop Requirements**
- Test: Run after collecting crop requirements via good-question
- Verify: Structured analysis output with recommendations and suggestions
...
1. Follow existing module patterns in `crop_planning/lib/` for structure and doctest format
...
**Doctest Isolation**: Use `tempfile.mkdtemp()` or explicit test paths in examples. Never assume `.claude/crop_planning/` directory structure exists during tests.
