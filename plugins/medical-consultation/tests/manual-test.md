# Medical Consultation Plugin - E2E Manual Test

Complete end-to-end test for adaptive symptom analysis.
...
## /medical-consultation:good-question
...
**Verify:**
```bash
grep -c "medical_consultation" .claude/settings.local.json
```
Should output: `9`
...
- Invokes `/medical-consultation:symptom-analysis` skill
...
Example: "See symptoms at https://example.com/spec.md"
...
**Setup:**
```bash
echo "invalid json" > .claude/medical_consultation/sessions/test-session.json
```
...
**Cleanup:**
```bash
rm .claude/medical_consultation/sessions/test-session.json
```
...
```bash
cat .claude/settings.local.json | jq '.permissions.allow[] | select(. | tostring | contains("medical_consultation"))'
```
...
8. Skill (medical-consultation:symptom-analysis)
9. Read (session data)
...
**Test init:**
```bash
mise run test:e2e:cli:medical-consultation:init
```
...
**Test next-question:**
```bash
SESSION_ID=<SESSION_ID> mise run test:e2e:cli:medical-consultation:next-question
```
...
rm -rf .claude/medical_consultation/sessions/
```

**Optional: Remove permissions (if needed)**
```bash
# Edit .claude/settings.local.json manually to remove medical_consultation entries
```
...
Then run `/medical-consultation:good-question` again
...
**Create directory:**
```bash
mkdir -p .claude/medical_consultation/sessions
```
