#!/usr/bin/env python3
"""
CLI for medical-consultation session management.
...
Session state is persisted to .claude/medical_consultation/sessions/<session_id>.json
"""
...
from medical_consultation.lib.dimension_belief import HypothesisSet
from medical_consultation.lib.session_orchestrator import SessionOrchestrator
...
def get_session_dir() -> Path:
    ...
    while current not in (current.parent, home):
        claude_dir = current / ".claude" / "medical_consultation" / "sessions"
        if claude_dir.parent.parent.exists():
            claude_dir.mkdir(parents=True, exist_ok=True)
            return claude_dir
        current = current.parent

    # Fallback: Use .medical_consultation in current directory (no workspace found)
    # This avoids polluting ~/.claude/ when run outside a workspace
    fallback = Path.cwd() / ".medical_consultation" / "sessions"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Medical consultation session management CLI"
    )
