#!/usr/bin/env python3
"""
Setup permissions for pc_repair plugin.
...
# Required permissions for pc_repair plugin
REQUIRED_PERMISSIONS = [
    # Environment setup
    'Bash(export PYTHONPATH="${CLAUDE_PLUGIN_ROOT}")',
    # CLI commands
    "Bash(python3 -m pc_repair.cli.session init*)",
    "Bash(python3 -m pc_repair.cli.session next-question*)",
    "Bash(python3 -m pc_repair.cli.session evaluate-question*)",
    "Bash(python3 -m pc_repair.cli.session update-with-computation*)",
    "Bash(python3 -m pc_repair.cli.session status*)",
    "Bash(python3 -m pc_repair.cli.session complete*)",
    # Skills (for post-session analysis only)
    "Skill(pc-repair:repair-analysis)",
    # Read permissions for session data
    "Read(.claude/pc_repair/sessions/*.json)",
]
