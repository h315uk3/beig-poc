# PC Repair Plugin - E2E Manual Test

Complete end-to-end test for adaptive PC repair diagnostics.
...
**Verify:**
```bash
grep -c "pc_repair" .claude/settings.local.json
```
Should output: `9`
...
• PC won't start or crashes
• Slow performance
• Hardware failure
• Other
```

**Action:** Answer naturally based on coherent scenario (e.g., "fixing a slow PC")

**Expected for each question:**
- Questions adapt to previous answers
- Clear and relevant
- No duplicates
- Header shows dimension (e.g., "Symptoms", "Hardware")
...
On next question, select: "Found solution"
...
- Invokes `/pc-repair:requirement-analysis` skill
...
**Setup:**
```bash
echo "invalid json" > .claude/pc_repair/sessions/test-session.json
```
...
**Cleanup:**
```bash
rm .claude/pc_repair/sessions/test-session.json
```
...
```bash
cat .claude/settings.local.json | jq '.permissions.allow[] | select(. | tostring | contains("pc_repair"))'
```
...
rm -rf .claude/pc_repair/sessions/
```

**Optional: Remove permissions (if needed)**
```bash
# Edit .claude/settings.local.json manually to remove pc_repair entries
```
...
Then run `/pc-repair:good-question` again
...
**Create directory:**
```bash
mkdir -p .claude/pc_repair/sessions
```
