...
Session state is persisted to .claude/smartphone_purchase/sessions/<session_id>.json
"""
...
from smartphone_purchase.lib.dimension_belief import HypothesisSet
from smartphone_purchase.lib.session_orchestrator import SessionOrchestrator
...
def get_session_dir() -> Path:
    ...
    while current not in (current.parent, home):
        claude_dir = current / ".claude" / "smartphone_purchase" / "sessions"
        if claude_dir.parent.parent.exists():
            claude_dir.mkdir(parents=True, exist_ok=True)
            return claude_dir
        current = current.parent

    # Fallback: Use .smartphone_purchase in current directory (no workspace found)
    # This avoids polluting ~/.claude/ when run outside a workspace
    fallback = Path.cwd() / ".smartphone_purchase" / "sessions"
