#!/usr/bin/env python3
"""
CLI for smartphone repair session management.
...
Session state is persisted to .claude/smartphone_repair/sessions/<session_id>.json
"""
...
from smartphone_repair.lib.dimension_belief import HypothesisSet
from smartphone_repair.lib.session_orchestrator import SessionOrchestrator
...
def get_session_dir() -> Path:
    ...
    while current not in (current.parent, home):
        claude_dir = current / ".claude" / "smartphone_repair" / "sessions"
        if claude_dir.parent.parent.exists():
            claude_dir.mkdir(parents=True, exist_ok=True)
            return claude_dir
        current = current.parent

    # Fallback: Use .smartphone_repair in current directory (no workspace found)
    # This avoids polluting ~/.claude/ when run outside a workspace
    fallback = Path.cwd() / ".smartphone_repair" / "sessions"
