# Smartphone Repair Plugin - E2E Manual Test

Complete end-to-end test for adaptive device repair diagnosis.
...
2. Typing `!` prefix directly in prompt (e.g., `!cat .claude/smartphone_repair/session_notes.local.md`)
...
## /smartphone-repair:good-question
...
**Verify:**
```bash
grep -c "smartphone_repair" .claude/settings.local.json
```
Should output: `9`
...
**Action:** Answer naturally based on coherent scenario (e.g., "using an iPhone")

**Expected for each question:**
- Questions adapt to previous answers
- Clear and relevant
- No duplicates
- Header shows dimension (e.g., "Device Info", "Symptoms")
...
- Invokes `/smartphone-repair:repair-analysis` skill
- Generates structured repair recommendations:
  - Device overview
  - Diagnosis summary
...
Example: "See device specs at https://example.com/spec.md"
...
Example:
- Q1: "What device?" → "iPhone 13"
- Q5: "What OS version?" → "Android 12"
...
**Setup:**
```bash
echo "invalid json" > .claude/smartphone_repair/sessions/test-session.json
```
...
**Cleanup:**
```bash
rm .claude/smartphone_repair/sessions/test-session.json
```
...
```bash
cat .claude/settings.local.json | jq '.permissions.allow[] | select(. | tostring | contains("smartphone_repair"))'
```
...
8. Skill (smartphone-repair:repair-analysis)
9. Read (session data)
...
**Test init:**
```bash
mise run test:e2e:cli:smartphone-repair:init
```
...
**Test next-question:**
```bash
SESSION_ID=<SESSION_ID> mise run test:e2e:cli:smartphone-repair:next-question
```
...
rm -rf .claude/smartphone_repair/sessions/
```

**Optional: Remove permissions (if needed)**
```bash
# Edit .claude/settings.local.json manually to remove smartphone_repair entries
```
...
Then run `/smartphone-repair:good-question` again
...
**Create directory:**
```bash
mkdir -p .claude/smartphone_repair/sessions
```
