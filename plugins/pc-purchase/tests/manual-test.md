# PC Purchase Plugin - E2E Manual Test

Complete end-to-end test for adaptive PC purchase decision support.
...
**Verify:**
```bash
grep -c "pc_purchase" .claude/settings.local.json
```
Should output: `9`
...
- Invokes `/pc-purchase:requirement-analysis` skill
...
**Setup:**
```bash
echo "invalid json" > .claude/pc_purchase/sessions/test-session.json
```
...
**Cleanup:**
```bash
rm .claude/pc_purchase/sessions/test-session.json
```
...
```bash
cat .claude/settings.local.json | jq '.permissions.allow[] | select(. | tostring | contains("pc_purchase"))'
```
...
8. Skill (pc-purchase:requirement-analysis)
9. Read (session data)
...
**Test init:**
```bash
mise run test:e2e:cli:pc-purchase:init
```
...
**Test next-question:**
```bash
SESSION_ID=<SESSION_ID> mise run test:e2e:cli:pc-purchase:next-question
```
...
rm -rf .claude/pc_purchase/sessions/
```

**Optional: Remove permissions (if needed)**
```bash
# Edit .claude/settings.local.json manually to remove pc_purchase entries
```
...
Then run `/pc-purchase:good-question` again
...
**Create directory:**
```bash
mkdir -p .claude/pc_purchase/sessions
```
