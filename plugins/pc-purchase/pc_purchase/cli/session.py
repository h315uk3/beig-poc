...
Session state is persisted to .claude/pc_purchase/sessions/<session_id>.json
"""
...
from pc_purchase.lib.dimension_belief import HypothesisSet
from pc_purchase.lib.session_orchestrator import SessionOrchestrator
...
def get_session_dir() -> Path:
    ...
    while current not in (current.parent, home):
        claude_dir = current / ".claude" / "pc_purchase" / "sessions"
        if claude_dir.parent.parent.exists():
            claude_dir.mkdir(parents=True, exist_ok=True)
            return claude_dir
        current = current.parent

    # Fallback: Use .pc_purchase in current directory (no workspace found)
    # This avoids polluting ~/.claude/ when run outside a workspace
    fallback = Path.cwd() / ".pc_purchase" / "sessions"
