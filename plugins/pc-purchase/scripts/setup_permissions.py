#!/usr/bin/env python3
"""
Setup permissions for pc-purchase plugin.
...
# Required permissions for pc-purchase plugin
REQUIRED_PERMISSIONS = [
    # Environment setup
    'Bash(export PYTHONPATH="${CLAUDE_PLUGIN_ROOT}")',
    # CLI commands
    "Bash(python3 -m pc_purchase.cli.session init*)",
    "Bash(python3 -m pc_purchase.cli.session next-question*)",
    "Bash(python3 -m pc_purchase.cli.session evaluate-question*)",
    "Bash(python3 -m pc_purchase.cli.session update-with-computation*)",
    "Bash(python3 -m pc_purchase.cli.session status*)",
    "Bash(python3 -m pc_purchase.cli.session complete*)",
    # Skills (for post-session analysis only)
    "Skill(pc-purchase:requirement-analysis)",
    # Read permissions for session data
    "Read(.claude/pc_purchase/sessions/*.json)",
]
